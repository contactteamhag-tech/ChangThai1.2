...
window.toggleMenu = window.toggleMenu || function () {
    ...
    if (burger) burger.setAttribute('aria-expanded', willOpen ? 'true' : 'false');
};
...
document.querySelectorAll('.reveal').forEach(el => {
    ...
    }, { threshold: .12, rootMargin: '0px 0px -40px 0px' }).observe(el);
});
...
document.querySelectorAll('a[href^="#"]').forEach(a => {
    a.addEventListener('click', function (e) {
        ...
        (function () {
            ...
            if (navbar) {
                const updateNavbarScroll = function () {
            ...
            backdrop.onclick = window.closeMenu;
            ...
            window.addEventListener('orientationchange', function () {
                setTimeout(syncForViewport, 120);
            });
...
// ===== FINAL MOBILE NAV HOTFIX v2 =====
(function(){
  function initFinalMobileNav(){
    var burger = document.getElementById('navToggle');
    var navLinks = document.getElementById('navLinks');
    var navbar = document.getElementById('navbar');
    if (!burger || !navLinks) return;

    if (!navLinks.dataset.movedToBody) {
      document.body.appendChild(navLinks);
      navLinks.dataset.movedToBody = 'true';
    }
    ...
    function setMenuState(open){
      var shouldOpen = !!open && mq.matches;
      navLinks.classList.toggle('open', shouldOpen);
    ...
    function syncViewport(){
      if (!mq.matches) window.closeMenu();
    }
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initFinalMobileNav, { once:true });
  } else {
    initFinalMobileNav();
  }
})();
